pipeline{
     ...
     stages {
         ...
         stage('TRIVY FS SCAN') {
             ...
             steps{
                 script{
                    withDockerRegistry(credentialsId: 'dockerhub', toolName: 'docker'){   
                        ...
                        sh "docker tag swiggy-clone zaid57/swiggy-clone:latest "
                        sh "docker push zaid57/swiggy-clone:latest "
                     }
                 }
             }
         }
          stage("TRIVY"){
             steps{
                 sh "trivy image zaid57/swiggy-clone:latest > trivyimage.txt" 
             }
         }
